...
def make_map():
    ...
    # users
    map.redirect('/users/{url:.*}', '/userkemenkeu/{url}')
    map.redirect('/userkemenkeu/', '/userkemenkeu')
    with SubMapper(map, controller='user') as m:
        m.connect('/userkemenkeu/edit', action='edit')
        # Note: openid users have slashes in their ids, so need the wildcard
        # in the route.
        m.connect('user_generate_apikey', '/userkemenkeu/generate_key/{id}', action='generate_apikey')
        m.connect('/userkemenkeu/activity/{id}/{offset}', action='activity')
        m.connect('user_activity_stream', '/userkemenkeu/activity/{id}',
                  action='activity', ckan_icon='clock-o')
        ...
        m.connect('user_follow', '/userkemenkeu/follow/{id}', action='follow')
        m.connect('/userkemenkeu/unfollow/{id}', action='unfollow')
        m.connect('user_followers', '/userkemenkeu/followers/{id:.*}',
                  action='followers', ckan_icon='users')
        m.connect('user_edit', '/userkemenkeu/edit/{id:.*}', action='edit',
                  ckan_icon='cog')
        m.connect('user_delete', '/userkemenkeu/delete/{id}', action='delete')
        m.connect('/userkemenkeu/reset/{id:.*}', action='perform_reset')
        m.connect('register', '/userkemenkeu/register', action='register')
        m.connect('login', '/userkemenkeu/login', action='login')
        m.connect('/user/_logout', action='logout')
        m.connect('/user/logged_in', action='logged_in')
        m.connect('/user/logged_out', action='logged_out')
        m.connect('/user/logged_out_redirect', action='logged_out_page')
        m.connect('/userkemenkeu/reset', action='request_reset')
        m.connect('/userkemenkeu/me', action='me')
        m.connect('/userkemenkeu/set_lang/{lang}', action='set_lang')
        m.connect('user_datasets', '/userkemenkeu/{id:.*}', action='read',
                  ckan_icon='sitemap')
